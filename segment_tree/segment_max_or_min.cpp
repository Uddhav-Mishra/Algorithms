...
void assign(int node, ll value) {
  tree[node].val = value;
}

segtree merge(segtree a, segtree b) {
  segtree combine;
  combine.val = max(a.val, b.val); // max/min 
  return combine;
}
...
void update_tree(int node, int l, int r, int index, ll val) {
...
void RandomizeCheck(int n, int test_cnt) {
  ...
  for (int z =1; z <= test_cnt; z++) {
    ...
    */
    if (query(1, 0, n-1, l, r).val != GetBruteMax(l,r)) {
      failed_cnt++;
    }
  }
